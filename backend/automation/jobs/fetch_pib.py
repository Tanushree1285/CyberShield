...
def fetch_pib_advisories():
    logging.info(f"Fetching PIB advisories from {PIB_URL}...")
    app = create_app()
    with app.app_context():
        ...
        try:
            response = requests.get(PIB_URL, verify=False, timeout=10)
            response.raise_for_status()
        except Exception as e:
            logging.error(f"Network error fetching PIB: {e}")
            return
        ...
        try:
            ...
            for link in links:
                ...
                if len(text) > 15 and ('cyber' in text.lower() or 'security' in text.lower() or 'digital' in text.lower()):
                    full_url = href if href.startswith('http') else f"https://pib.gov.in/{href}"
                    parsed_date = datetime.utcnow()
                    import re
                    from dateutil import parser
                    
                    parent_text = link.parent.parent.get_text() if link.parent and link.parent.parent else link.parent.get_text() if link.parent else text
                    ...
                    if not Article.query.filter_by(url=full_url).first():
                        new_article = Article(
                            title=text,
                            content="Recent press release from Press Information Bureau (India).",
                            url=full_url,
                            source="Press Information Bureau",
                            country_id=india.id,
                            type=detect_type(text, "cyber awareness"),
            ...
            if advisories_added > 0:
                db.session.commit()
                logging.info(f"Added {advisories_added} PIB articles.")
        except Exception as e:
            logging.error(f"Error parsing PIB layout: {e}")
